...
namespace WzComparerR2.CharaSimControl
{
    public class GearTooltipRender : TooltipRender
    {
        ...
        private Bitmap renderBase(out int picHeight, out int iconY)
        {
            ...
            g.DrawString("Beginner", GearGraphics.ItemDetailFont, reqJob > 0 ? Brushes.Red : Brushes.White, 10, picHeight);
            ...
            g.DrawString("Archer", GearGraphics.ItemDetailFont, (reqJob & 4) == 0 ? Brushes.Red : Brushes.White, 130, picHeight);
            ...
            {
                g.DrawString("Very True", GearGraphics.ItemNameFont, GearGraphics.SetItemNameBrush, 126, picHeight, format);
                picHeight += 18;
            }
            if (gear.Props.TryGetValue(GearPropType.limitBreak, out value) && value > 0)
            {
                g.DrawString("Limit Break", GearGraphics.ItemNameFont, GearGraphics.SetItemNameBrush, 126, picHeight, format);
                picHeight += 18;
            }
            ...
            if (!string.IsNullOrEmpty(typeStr))
            {
                ...
                g.DrawString((isWeapon ? "Weapon" : "Equip") + "Classification : " + typeStr,
            ...
            if (hasTuc)
            {
                g.DrawString("Available upgrades : " + value + "回", GearGraphics.ItemDetailFont, Brushes.White, 8, picHeight);
                picHeight += 16;
                hasPart2 = true;
            }
            if (gear.Props.TryGetValue(GearPropType.limitBreak, out value) && value > 0)
            ...
            if (hasTuc && gear.Hammer > -1)
            {
                if (gear.Hammer == 2)
                {
                    g.DrawString("Hammer complete", GearGraphics.ItemDetailFont, Brushes.White, 8, picHeight);
                    picHeight += 16;
                }
                if (gear.Props.TryGetValue(GearPropType.superiorEqp, out value) && value > 0)
                {
                ...
                if (gear.Star > 0)
                {
                    g.DrawString("Apply" + gear.Star + "star enhancement", GearGraphics.ItemDetailFont, GearGraphics.OrangeBrush, 8, picHeight);
                    picHeight += 16;
                }
                picHeight += 2;
                g.DrawString("Hammer has increased the number of slots", GearGraphics.ItemDetailFont, GearGraphics.GoldHammerBrush, 8, picHeight);
            ...
            {
                GearGraphics.DrawString(g, " #cPotential scrolls# can #cCincrease# the potential of items，but you need to identify them with a #cmagnifying lens#.\n Use the #cmagnifying lens# to #creveal# item potential",
            ...
            if (adOptionCount > 0)
            {
                ...
                g.DrawString("Additional Potential", GearGraphics.ItemDetailFont, GearGraphics.SetItemNameBrush, 26, picHeight+2);
                picHeight += 24;
            ...
            if (gear.State == GearState.itemList)
            {
                ...
                {
                    picHeight -= 5;
                    GearGraphics.DrawString(g, "\n #cEquipped with" + incline.Substring(1) + "experience，only 1 time.#",
                        GearGraphics.ItemDetailFont, 8, 236, ref picHeight, 16);
                    picHeight += 8;
                }
            }
        ...
        private Bitmap renderSetItem(out int picHeight)
        {
            ...
            if (gear.Props.TryGetValue(GearPropType.setItemID, out setID))
            {
                ...
                {
                    g.DrawString(effect.Key + "Set Effect", GearGraphics.ItemDetailFont, GearGraphics.SetItemNameBrush, 8, picHeight);
                    picHeight += 16;
