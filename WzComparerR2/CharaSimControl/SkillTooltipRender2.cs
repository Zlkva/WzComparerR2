...
namespace WzComparerR2.CharaSimControl
{
    public class SkillTooltipRender2 : TooltipRender
    {
        ...
        private Bitmap RenderSkill(out int picH)
        {
            ...
            picH = 35;
            if (!Skill.PreBBSkill)
                GearGraphics.DrawString(g, "[Max Level：" + Skill.MaxLevel + "]", GearGraphics.ItemDetailFont2, 90, 270, ref picH, 16);
            ...
            {
                string hStr = SummaryParser.GetSkillSummary(Skill, Skill.Level, sr, SummaryParams.Default);
                GearGraphics.DrawString(g, "[Level " + Skill.Level + "]", GearGraphics.ItemDetailFont, 8, 272, ref picH, 16);
                if (hStr != null)
                {
            ...
            if (ShowProperties)
            {
                ...
                if (Skill.Invisible)
                {
                    attr.Add("Hidden Skills");
                }
                if (Skill.Hyper != HyperSkillType.None)
                {
                    attr.Add("Hyper Skills:" + Skill.Hyper);
                }
                ...
                if (Skill.NotRemoved)
                {
                    attr.Add("Cannot be removed");
                }
                if (Skill.MasterLevel > 0 && Skill.MasterLevel < Skill.MaxLevel)
                {
                    attr.Add("Initial Mastery :Lv." + Skill.MasterLevel);
                }
            ...
            if (ShowReqSkill && Skill.ReqSkill.Count > 0)
            {
                foreach (var kv in Skill.ReqSkill)
                {
                    ...
                    skillDescEx.Add("#c[Pre-skills] " + skillName + ": " + kv.Value + " 级#");
                }
            }
