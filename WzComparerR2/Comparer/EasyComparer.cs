...
namespace WzComparerR2.Comparer
{
    public class EasyComparer
    {
        ...
        private void OutputFile(List<Wz_File> fileNew, List<Wz_File> fileOld, Wz_Type type, List<CompareDifference> diffLst, string outputDir)
        {
            ...
            StateInfo = "Comparing files..." + type;
            StateDetail = "Output file is being constructed";
            try
            {
                ...
                sw.WriteLine("<table>");
                sw.WriteLine("<tr><th>&nbsp;</th><th>File Name</th><th>File Size</th><th>File Version</th></tr>");
                ...
                sw.WriteLine("<tr><td>Time</td><td colspan='3'>{0:yyyy-MM-dd HH:mm:ss.fff}</td></tr>", DateTime.Now);
                sw.WriteLine("<tr><td>Parameter</td><td colspan='3'>{0}</td></tr>", string.Join("<br/>", new[] {
                ...
                for (int i = 0; i < sb.Length; i++)
                {
                    ...
                    sw.WriteLine("<tr><th>{0} of {1} total items</th></tr>", diffStr[i], count[i]);
                ...
                foreach (CompareDifference diff in diffLst)
                {
                    switch (diff.DifferenceType)
                    {
                        case DifferenceType.Changed:
                            {
                                StateInfo = string.Format("{0}/{1} outputting MODIFIED {2}", count[0], count[3], diff.NodeNew.FullPath);
                        ...
                        case DifferenceType.Append:
                            if (this.OutputAddedImg)
                            {
                                StateInfo = string.Format("{0}/{1} outputting NEW {2}", count[1], count[4], diff.NodeNew.FullPath);
                        ...
                        case DifferenceType.Remove:
                            if (this.OutputRemovedImg)
                            {
                                StateInfo = string.Format("{0}/{1} outputting REMOVED {2}", count[2], count[5], diff.NodeOld.FullPath);
        ...
        private void CompareImg(Wz_Image imgNew, Wz_Image imgOld, string imgName, string anchorName, string menuAnchorName, string outputDir, StreamWriter sw)
        {
            ...
            int[] count = new int[3];
            StateDetail = "Statistical overview and output resource files ... Changes in total" + diffList.Count;
            ...
            sw.WriteLine("<tr><th colspan=\"3\"><a name=\"{1}\">{0}</a> Modified:{2} New:{3} Removed:{4}</th></tr>",
                imgName, anchorName, count[0], count[1], count[2]);
            sw.WriteLine(sb.ToString());
            sw.WriteLine("<tr><td colspan=\"3\"><a href=\"#{1}\">{0}</a></td></tr>", "Back to Index", menuAnchorName);
            sw.WriteLine("</table>");
            imgNew.Unextract();
            imgOld.Unextract();
            sb = null;
        ...
        private void OutputImg(Wz_Image img, DifferenceType diffType, string imgName, string anchorName, string menuAnchorName, string outputDir, StreamWriter sw)
        {
            ...
            StateDetail = "Outputting full IMG structure";
            sw.WriteLine("<table class=\"img\">");
            sw.WriteLine("<tr><th colspan=\"2\"><a name=\"{1}\">wz_image: {0}</a></th></tr>", imgName, anchorName);
            fnOutput(img.Node);
            sw.WriteLine("<tr><td colspan=\"2\"><a href=\"#{1}\">{0}</a></td></tr>", "Back to Index", menuAnchorName);
            sw.WriteLine("</table>");
            img.Unextract();
        }
